"""
build_rag_collections.py  —  Capstone Project 8 · RAG Ingestion Pipeline
=========================================================================
...
THREE COLLECTIONS (vs one monolithic collection in rag_utils.py)
-----------------------------------------------------------------
...
collection built by rag_utils.py (which ingests the Excel workbook).
...
#   __file__        → scripts/build_rag_collections.py
#   .parent         → scripts/
#   .parent.parent  → project root  (where data/, outputs/, src/ live)
PROJECT_ROOT = Path(__file__).resolve().parent.parent

# Where raw knowledge files live (one subdirectory per collection)
RAG_DATA_ROOT = PROJECT_ROOT / "data" / "raw" / "RAG_data"

# Where ChromaDB persists its SQLite + HNSW index files
CHROMA_DIR = PROJECT_ROOT / "outputs" / "chromadb"

# Embedding model loaded via sentence-transformers.
# all-MiniLM-L6-v2 is a 22M-parameter distilled model:
#   - Produces 384-dimensional unit vectors (L2 norm = 1 after normalization)
#   - Trained on 1B+ sentence pairs from diverse English text
#   - Fast enough to run on CPU; no GPU required for capstone workloads
...
def query_collection(
    ...
    # Reconstruct the embedding function with identical settings to ingest time.
    # normalize_embeddings=True ensures the stored and query vectors share
    # the same unit-sphere geometry — essential for cosine distance correctness.
...
def main() -> None:
    ...
    # One shared ChromaDB client for all three collections —
    # the client is just a connection handle to the persistent directory.
    client = chromadb.PersistentClient(path=str(CHROMA_DIR))

    # One shared embedding function instance.
    # SentenceTransformerEmbeddingFunction downloads & caches the MiniLM model
    # on first call, then keeps it in memory for the rest of the process.
    embed_fn = SentenceTransformerEmbeddingFunction(
        model_name=EMBEDDING_MODEL,
        normalize_embeddings=True,  # output vectors are unit-length → cosine = dot product
    )

    # Print configuration so it's clear what we're doing
    print(f"\nEmbedding model : {EMBEDDING_MODEL}  (384-dim, cosine space)")
